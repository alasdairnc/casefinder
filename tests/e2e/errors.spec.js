...
test.describe("Error handling", () => {
  test("shows error when API returns 500", async ({ page }) => {
    ...
    await expect(
      page.locator("text=/error|something went wrong|try again/i").first(),
  ...
  test("shows rate limit message when API returns 429", async ({ page }) => {
    await page.route("/api/analyze", async (route) => {
      await route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          error: "Rate limit reached. Try again in 3 minutes.",
        }),
    ...
    await page.locator('[data-testid="research-submit"]').click();
    await expect(
      page.locator("text=/rate limit|try again|minutes/i").first(),
    ).toBeVisible({ timeout: 10000 });
  });

  test("research button is disabled when textarea is empty", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("character counter updates when typing in textarea", async ({
    page,
  }) => {
    await page.goto("/");
