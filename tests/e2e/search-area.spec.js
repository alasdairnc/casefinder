...
test.describe("SearchArea", () => {
  ...
  test("renders correctly with input field and submit button", async ({
    page,
  }) => {
    ...
    await expect(input).toHaveAttribute(
      "placeholder",
      /Describe your legal scenario in plain language/,
    );
  });

  test("accepts text correctly and character count updates", async ({
    page,
  }) => {
    const input = page.getByTestId("scenario-input");

    const scenario =
      "A suspect was observed entering a residential property at night.";
